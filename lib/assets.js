...
sproto.preprocessContent = function() {
  if (this.cr) {
    this.cr.setContent(stylus.render(this.cr.getContent('utf8')));
    cproto.preprocessContent.call(this);
  }
}
...
exports.parse = function parse(route, context, servePath) {
  ...
  if(ext === 'css' || ext === 'less' || ext === 'styl') {
    ...
    if(metaData.path.indexOf(".styl") != -1) {//make sure it's not really a less file reference
      ext = "styl";
  ...
  switch(ext) {
    ...
    case "styl":
      return new StylusAsset(metaData, context, serverPath);
    case "css":
