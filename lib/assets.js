...
/**
 * ReworkAsset Object definiiton
 */

function ReworkAsset(metaData, context, servePath) {
  if (metaData) {
    CSSAsset.call(this, metaData, 'styl', context, servePath);
  }
}
var rproto = ReworkAsset.prototype = new CSSAsset;

rproto.getRelativePath = function() {
  var relPath = cproto.getRelativePath.call(this);
  relPath = relPath.replace(".styl", ".styl.css");
  return relPath;
};

rproto.getRequestPath = function() {
  var reqPath = cproto.getRequestPath.call(this);
  reqPath = reqPath.replace(".styl", ".styl.css");
  return reqPath;
};

rproto.preprocessContent = function() {
  if (this.cr) {
    this.cr.setContent(rework(this.cr.getContent('utf8')).toString());
...
exports.parse = function parse(route, context, servePath) {
  ...
  if(ext === 'css' || ext === 'less' || ext === 'styl') {
    ...
    if(metaData.path.indexOf(".style") != -1) {//make sure it's not really a less file reference
      ext = "styl";
  ...
  switch(ext) {
    ...
    case "styl":
      return new ReworkAsset(metaData, context, serverPath);
    case "css":
