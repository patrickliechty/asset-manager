  ...
  if(['css', 'js', 'img'].indexOf(pathParts[0]) !== -1) {
    ...
    if(asset){
      if(asset.type === 'css') {
        ...
        res.send(content, {'Content-Type' : 'text/javascript'});
      } else {
...
function karmaPreprocessor(config, file) {
  init(config);
  assets.init(config.paths, false, false);
  var asset = assets.parse('modules/FanChartControl.js', context, servePath);
    ...
    start(config, function startCB(){
      ...
      async.series([
        ...
        var files = utils.filterAssembliesFiles(fileSets[0], fileSets[1]),
            moduleCSSFiles = utils.findModuleCSSFiles(fileSets[1]) || [];
...
module.exports.karmaPreprocessor = karmaPreprocessor;