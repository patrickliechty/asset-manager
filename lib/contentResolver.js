  ...
  if(meta.assembled) {
    try {
      ...
      var enFilePath = path.join(meta.basePath, "../locales", meta.pathPart, meta.name + "_en.json"),
          localeExists = fs.existsSync(filePath),
      ...
      filePath = path.join(meta.baseModulePath, meta.name + ".css");//look in the Module folder
      if(!fs.existsSync(filePath)) {
        filePath = path.join(meta.basePath, "css", meta.pathPart, meta.name + ".css");//look in the css folder
      }
