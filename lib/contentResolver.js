...
function readContentSync(meta, isSubAssembly) {
  ...
  if(meta.assembled) {
    try {
      ...
      if (assembly.files && assembly.files.length > 0) {
        contents = "//Module assembly: " + meta.name + "\n\n";

        if (isSubAssembly && assembly.simpleWrap !== true) {
        ...
        var baseTemplatePath = path.join(meta.baseModulePath, "templates");
        var templatesStr = "";
        if (fs.existsSync(baseTemplatePath)) {
          var fileList = fs.readdirSync(baseTemplatePath);
          ...
          fileList.forEach(function(fileName){
            if (fileName.slice(-5).toLowerCase() === ".html") {
              ...
              var tempateKey = fileName.slice(0,-5);

              if(invalidKeyTest.test(tempateKey)){
                throw new Error("Template file names can only use _ or alphanumeric characters.")
              }

              templatesStr = "templateList."+tempateKey+" = "+str+";";
              contents = appendContents(contents, templatesStr, 'templates/'+fileName);
            }
          });
        }
        ...
        if (assembly.assemblies && assembly.assemblies.length > 0) {
          contents = "";
        }
      }
      ...
      if (assembly.assemblies) {
        var len = assembly.assemblies.length;
        for(i=0; i<len; ++i) {
