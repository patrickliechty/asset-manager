  ...
  if(meta.assembled) {
    try {
      ...
      var enFilePath = path.join(meta.basePath, "../locales", meta.pathPart, meta.name + "_en.json"),
          enLocaleExists = fs.existsSync(enFilePath);
      hasTranslations = enLocaleExists;
      if(hasTranslations) {
        var enLocaleVal = enLocaleExists ? fs.readFileSync(enFilePath).toString('utf8') : "{}";
        enLocaleVal = JSON.parse(enLocaleVal);
        
        translations = { 'en': enLocaleVal };
        
        contents = appendContents(contents, "var langs = " + JSON.stringify(translations) + ";", meta.name + "_en" + ".json");
        contents = appendContents(contents, "var lang = langs['en'];", "Injected code");
      }
...
function getContentMeta(pathPart, name, ext, type) {
  ...
  for(var i=0; i<searchPaths.length; ++i){
    ...
    if(type === 'css') {
      ...
      fullPath = path.resolve(searchPaths[i], moduleCSSPath);
      //look for exact path match
      if(fs.existsSync(fullPath)) {
        meta.mainFile = fullPath;
        break;
  ...
  if(meta.mainFile === null) {
    console.log("Unrecognized asset: " + checkPath);
